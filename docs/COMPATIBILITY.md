...
Stable recreation mode preserves the established ore and landmark placement sequence. Rotating mode takes effect only when initialization or recreation commits; its derived salt is persisted, so ordinary restarts cannot silently move resources or landmarks in chunks generated later.

Lifecycle-owned fields are locked during a live config reload. Delvefold keeps the active last-known-good snapshot if an edit attempts to replace the generation epoch, derived salt, terrain, terrain scale, initialization state, or committed operation ID; restoring those values requires the normal restart-safe world or backup workflow.
