...
class ConfigIssueMessagesTest {
    ...
    void preservesMachineCodeAndPathWhileLocalizingTheDisplayDetail() {
        ...
        AdminLocalizedMessage.Decoded row =
                AdminLocalizedMessage.decode(ConfigIssueMessages.encode(issue)).orElseThrow();
        ...
        assertEquals(
                "message.delvefold.config_issue.severity.error",
                AdminLocalizedMessage.decode(row.arguments().get(0))
                        .orElseThrow()
                        .translationKey());
        assertEquals(
                "message.delvefold.config_issue.detail.selection_required",
                AdminLocalizedMessage.decode(row.arguments().get(3))
                        .orElseThrow()
                        .translationKey());
    }
    ...
    void retainsTechnicalDetailsOnlyForOperationalLoadFailures() {
        ...
        AdminLocalizedMessage.Decoded row =
                AdminLocalizedMessage.decode(ConfigIssueMessages.encode(issue)).orElseThrow();
        AdminLocalizedMessage.Decoded detail =
                AdminLocalizedMessage.decode(row.arguments().get(3)).orElseThrow();
