...
class OreImportViewsTest {
    ...
    void emptyViewsUseOneCompletePage() {
        assertDoesNotThrow(() -> new ScanView("token", 0L, "base", 0, 1,
                0, 0, false, List.of()));
        assertDoesNotThrow(() -> new PreviewView("token", 0L, "base", 0, 1,
                0, true, 0L, List.of(), List.of(), List.of(), false));
    }

    @Test
    void scanRejectsOutOfRangeAndIncompletePages() {
        assertThrows(IllegalArgumentException.class, () -> new ScanView(
                "token", 0L, "base", 1, 1, 0, 0, false, List.of()));
        assertThrows(IllegalArgumentException.class, () -> new ScanView(
                "token", 0L, "base", 0, 2, 0, 0, false, List.of()));
        assertThrows(IllegalArgumentException.class, () -> new ScanView(
                "token", 0L, "base", 0, 1, 1, 0, false, List.of()));
    }
    ...
    void scanRejectsDuplicateGroupAndCandidateIds() {
        ...
        assertThrows(IllegalArgumentException.class, () -> new ScanView(
                "token", 0L, "base", 0, 1, 2, 2, false, List.of(group, group)));
        assertThrows(IllegalArgumentException.class, () -> group(
                "example:tin", List.of(candidate("example:tin_ore"), candidate("example:tin_ore"))));
    }

    @Test
    void previewRejectsOutOfRangeAndIncompletePages() {
        assertThrows(IllegalArgumentException.class, () -> new PreviewView(
                "token", 0L, "base", 1, 1, 0, true, 0L,
                List.of(), List.of(), List.of(), false));
        assertThrows(IllegalArgumentException.class, () -> new PreviewView(
                "token", 0L, "base", 0, 2, 0, true, 0L,
                List.of(), List.of(), List.of(), false));
        assertThrows(IllegalArgumentException.class, () -> new PreviewView(
                "token", 0L, "base", 0, 1, 1, true, 0L,
                List.of(), List.of(), List.of(), false));
    }
    ...
    private static CandidateView candidate(String id) {
        return new CandidateView(id, HostKind.STONE.replaceTag(),
                HostKind.STONE, Evidence.CONVENTIONAL_TAG);
    }
}
