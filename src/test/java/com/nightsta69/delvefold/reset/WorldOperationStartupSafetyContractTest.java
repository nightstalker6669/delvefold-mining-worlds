...
class WorldOperationStartupSafetyContractTest {
    private static final Path RESET_ROOT = Path.of(
            "src/main/java/com/nightsta69/delvefold/reset");

    @Test
    void pendingDeleteOrRecreateCannotContinueStartupAfterPreparationOrFinalizationFailure()
            throws Exception {
        ...
        assertTrue(finish.contains("throw new IllegalStateException("),
    ...
    @Test
    void pendingRestoreUsesTransactionalBackupAndAbortsBeforeRestoredPhaseOnFailure()
            throws Exception {
        String restore = Files.readString(RESET_ROOT.resolve("WorldRestoreService.java"));
        String prepare = between(restore,
                "public void prepareStartup(MinecraftServer server)",
                "private static void auditBackupManifest");
