...
class DelvefoldAuditServiceTest {
    ...
    void resolvesContainedAuditDirectoryAndContainsUnavailableWrites() throws Exception {
        ...
        String source =
                Files.readString(Path.of("src/main/java/com/nightsta69/delvefold/audit/DelvefoldAuditService.java"));
        ...
        assertTrue(
                source.contains("return current.offer(captured);"),
                "record must only enqueue the immutable mutation, never append on its caller thread");
        assertTrue(
                source.contains("closeContained(current);"),
