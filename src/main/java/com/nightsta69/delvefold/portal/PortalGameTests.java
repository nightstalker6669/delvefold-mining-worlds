...
public final class PortalGameTests {
    ...
    public static void centralHubBuildsAStablePlatformAndReturnPortal(GameTestHelper helper) {
        ...
        helper.assertTrue(returnPortal.isFilled(helper.getLevel()),
                "Central hub did not create its guaranteed return portal");
        helper.assertTrue(helper.getLevel().getBlockState(floorCenter.offset(2, 0, 2))
        ...
        helper.assertTrue(helper.getLevel().getBlockState(
                        floorCenter.offset(-CentralHubService.PLATFORM_RADIUS, 0,
                                -CentralHubService.PLATFORM_RADIUS)).is(Blocks.LODESTONE),
                "Central hub did not persist its deterministic floor marker");
        helper.assertTrue(helper.getLevel().getBlockState(floorCenter.offset(3, 3, 3)).isAir(),
                "Central hub did not clear safe headroom around the portal");
        helper.assertTrue(CentralHubService.isIntact(helper.getLevel(), initial),
        ...
        CentralHubService.HubPlan recovered = CentralHubService.plan(helper.getLevel(),
        ...
        helper.assertTrue(recovered.floorCenter().equals(floorCenter),
                "Hub markers did not retain the selected vertical position");
        helper.assertTrue(rebuilt.isFilled(helper.getLevel()),
                "Central hub did not repair its return portal");
        helper.succeed();
    }
    ...
    public static void portalRoutingRemainsPlayerOnly(GameTestHelper helper) {
        ...
        var transition = PortalRegistries.PORTAL.get().getPortalDestination(
                helper.getLevel(), pig, helper.absolutePos(new BlockPos(1, 1, 1)));
    ...
    public static void centralHubEventsDenyOrdinaryPlayersAndAllowWorldManagers(GameTestHelper helper) {
        ...
        PortalSettings settings = new PortalSettings(true, true, 5, 1.0D,
        ...
        try {
            helper.assertTrue(!AdminAccess.canManageWorld(player),
            ...
            helper.assertTrue(deniedBreak.isCanceled(),
                    "Central-hub protection did not cancel an ordinary player's block break");
            ...
            helper.assertTrue(deniedPlace.isCanceled(),
                    "Central-hub protection did not cancel an ordinary player's block placement");

            BlockHitResult hit = new BlockHitResult(Vec3.atCenterOf(protectedPosition),
                    Direction.UP, protectedPosition, false);
            PlayerInteractEvent.RightClickBlock deniedInteraction =
                    new PlayerInteractEvent.RightClickBlock(
                            player, InteractionHand.MAIN_HAND, protectedPosition, hit);
            CentralHubProtectionEvents.applyRightClick(deniedInteraction, settings, true);
            helper.assertTrue(deniedInteraction.isCanceled()
            ...
            helper.assertTrue(AdminAccess.canManageWorld(player),
                    "An operator did not receive world-management permission");
            ...
            helper.assertTrue(!allowedBreak.isCanceled(),
                    "Central-hub protection canceled a world manager's block break");
            ...
            helper.assertTrue(!allowedPlace.isCanceled(),
                    "Central-hub protection canceled a world manager's block placement");
            ...
            helper.assertTrue(!allowedInteraction.isCanceled(),
                    "Central-hub protection canceled a world manager's interaction");
        } finally {
            playerList.deop(player.getGameProfile());
            player.discard();
        }
