...
public final class LandmarkGameTests {
    ...
    public static void catalogCodecLoadsEveryBundledDefinition(GameTestHelper helper) throws Exception {
        ...
        for (String id : BUNDLED_IDS) {
            ...
            try (var input = LandmarkGameTests.class.getResourceAsStream(path)) {
                ...
                LandmarkDefinition definition =
                        LandmarkDefinition.from(ResourceLocation.fromNamespaceAndPath(Delvefold.MOD_ID, id), body);
                ...
                JsonElement encoded = LandmarkDefinition.BODY_CODEC
                        .encodeStart(JsonOps.INSTANCE, body)
                        .getOrThrow(message -> new AssertionError(id + ": " + message));
                helper.assertTrue(
                        encoded.getAsJsonObject().has("placement_style"),
        ...
        for (TerrainMode terrain : TerrainMode.values()) {
            ...
            for (TerrainVariant variant : TerrainVariant.values()) {
                ...
                helper.assertTrue(
                        dimensionIds.add(levelKey.location()),
                ...
                try (var input = LandmarkGameTests.class.getResourceAsStream(path)) {
                    ...
                    var dimension = JsonParser.parseReader(new InputStreamReader(input, StandardCharsets.UTF_8))
                            .getAsJsonObject();
                    helper.assertTrue(
                            dimension.has("type")
                                    && dimension.has("generator")
        ...
        helper.assertTrue(
                dimensionIds.size() == TerrainMode.values().length * TerrainVariant.values().length,
    ...
    public static void landmarkPresetAcceptanceAndSelectionAreDeterministic(GameTestHelper helper) {
        ...
        for (long seed : seedForBucket) {
            ...
            helper.assertTrue(
                    !LandmarkSelector.accepts(LandmarkPreset.PURE_MINING, seed), "Pure Mining accepted a candidate");
        }
        ...
        LandmarkCatalogSnapshot snapshot = new LandmarkCatalogSnapshot(
                1L, Map.of(survey.id(), survey, motherlode.id(), motherlode), Instant.EPOCH);
        WorldIdentitySettings surveyOnly =
                WorldIdentitySettings.defaults().withLandmarks(LandmarkPreset.BALANCED, true, false, false);
        ...
        helper.assertTrue(
                first.equals(repeated) && first.orElseThrow() == survey,
                "Selection was not deterministic or ignored category toggles");
        helper.assertTrue(
                LandmarkSelector.categoryEnabled(LandmarkCategory.SURVEY_STATION, surveyOnly),
                "Survey toggle did not enable survey landmarks");
        helper.assertTrue(
                !LandmarkSelector.categoryEnabled(LandmarkCategory.MOTHERLODE, surveyOnly)
    ...
    public static void structureCatalogAndTemplatesLoadThroughVanillaRegistries(GameTestHelper helper) {
        ...
        helper.assertTrue(
                structure instanceof DelvefoldLandmarkStructure,
                "Bundled structure did not load with Delvefold's structure type");
        StructureSet set = helper.getLevel()
                .registryAccess()
        ...
        helper.assertTrue(
                set.placement() instanceof GenerationSaltedRandomSpreadPlacement,
                "Structure set did not load the generation-salted placement codec");
        GenerationSaltedRandomSpreadPlacement placement = (GenerationSaltedRandomSpreadPlacement) set.placement();
        helper.assertTrue(
                placement.spacing() == 12 && placement.separation() == 6,
        ...
        helper.assertTrue(
                multiChunkTemplate, "Bundled templates do not exercise structure-system multi-chunk placement");
        helper.succeed();
    }
    ...
    public static void rotationsKeepEveryBundledTemplateCenteredOnItsCandidate(GameTestHelper helper) {
        ...
        for (String id : BUNDLED_IDS) {
            ...
            for (Rotation rotation : Rotation.values()) {
                BlockPos origin =
                        DelvefoldLandmarkStructure.centeredOrigin(template, rotation, centerX, minimumY, centerZ);
                var bounds = template.getBoundingBox(origin, rotation, BlockPos.ZERO, Mirror.NONE);
                helper.assertTrue(
                        bounds.getCenter().getX() == centerX
                                && bounds.getCenter().getZ() == centerZ
                                && bounds.minY() == minimumY,
                        id + " shifted away from its candidate at rotation " + rotation + ": " + bounds);
            }
        }
        helper.succeed();
    }
    ...
    public static void fixedMiningBiomesResolveToTheirOwnTerrain(GameTestHelper helper) {
        ...
        for (TerrainMode terrain : TerrainMode.values()) {
            var biome = biomes.getHolderOrThrow(com.nightsta69.delvefold.world.DelvefoldWorldgen.biomeFor(terrain));
            helper.assertTrue(
                    com.nightsta69.delvefold.world.DelvefoldWorldgen.terrainFor(biome) == terrain,
    ...
    public static void capturedCatalogRevisionAndPlacementProbeCacheRemainCoherent(GameTestHelper helper) {
        ...
        LandmarkDefinition revisionTwo = definition("shared", "template_v2", LandmarkPlacementStyle.BURIED, -4, 4);
        LandmarkCatalogSnapshot first =
                new LandmarkCatalogSnapshot(1L, Map.of(revisionOne.id(), revisionOne), Instant.EPOCH);
        LandmarkCatalogSnapshot second =
        ...
        var selected = LandmarkGenerationPlanner.select(
                        captured,
                        TerrainMode.FLAT,
                        WorldIdentitySettings.defaults().withLandmarks(LandmarkPreset.ABUNDANT, true, true, true),
                        definition -> {
                ...
                .orElseThrow();
        helper.assertTrue(
                selected.definition() == revisionOne,
                "A catalog reload changed the selected definition after revision capture");
        helper.assertTrue(
                "test:template_v1".equals(selected.candidate()),
                "A candidate from one catalog revision was paired with another revision");
        helper.assertTrue(
                published.get() == second, "The regression did not simulate publication during candidate resolution");

        LandmarkPlacementProbeCache cache = new LandmarkPlacementProbeCache();
        LandmarkDefinition surfaceA = definition("surface_a", "surface_a", LandmarkPlacementStyle.SURFACE, -32, 96);
        LandmarkDefinition surfaceB = definition("surface_b", "surface_b", LandmarkPlacementStyle.SURFACE, -32, 96);
        ...
        helper.assertTrue(
                firstHeight == 73 && repeatedHeight == 73 && surfaceCalls.get() == 1 && cache.cachedProbeCount() == 1,
                "Definition-independent surface probes were not safely shared");

        LandmarkDefinition buried = definition("buried", "buried", LandmarkPlacementStyle.BURIED, -32, 96);
        AtomicInteger buriedCalls = new AtomicInteger();
        helper.assertTrue(
                cache.resolve(buried, buriedCalls::incrementAndGet) == 1
    ...
    public static void lootMarkersInstallTheirTableOnlyOnce(GameTestHelper helper) {
        ...
        ResourceKey<LootTable> loot = ResourceKey.create(
                Registries.LOOT_TABLE, ResourceLocation.fromNamespaceAndPath(Delvefold.MOD_ID, "chests/survey_camp"));
        helper.assertTrue(
                LandmarkTemplatePiece.installLootOnce(helper.getLevel(), position, loot, 12345L),
                "First loot-marker application failed");
        helper.assertTrue(
                !LandmarkTemplatePiece.installLootOnce(helper.getLevel(), position, loot, 67890L),
                "Second loot-marker application overwrote the container");
        RandomizableContainer container =
                (RandomizableContainer) helper.getLevel().getBlockEntity(position);
        helper.assertTrue(
                loot.equals(container.getLootTable()) && container.getLootTableSeed() == 12345L,
        ...
        helper.assertTrue(
                !LandmarkTemplatePiece.installLootOnce(helper.getLevel(), position, loot, 24680L),
    ...
    public static void persistedGenerationSaltRotatesIndependentLandmarkDomains(GameTestHelper helper) {
        ...
        helper.assertTrue(
                LandmarkSeeds.selectionSeed(worldSeed, chunk, 0L) != LandmarkSeeds.selectionSeed(worldSeed, chunk, 99L),
                "Generation salt did not rotate selection");
        helper.assertTrue(
                LandmarkSeeds.definitionSeed(worldSeed, chunk, 0L, id)
    ...
    public static void discoveryVisitTransitionsDeduplicateAndAdvancementIsServerAuthorized(GameTestHelper helper) {
        ...
        helper.assertTrue(
                LandmarkDiscoveryService.isVisitTransition(null, camp),
                "Entering a landmark after no visit did not trigger discovery");
        helper.assertTrue(
                !LandmarkDiscoveryService.isVisitTransition(camp, sameCamp),
                "A continuous visit triggered discovery twice");
        helper.assertTrue(
                LandmarkDiscoveryService.isVisitTransition(camp, grotto),
                "Moving into a different landmark did not trigger discovery");
        helper.assertTrue(
                LandmarkDiscoveryService.isVisitTransition(null, sameCamp),
                "Leaving and re-entering a landmark did not trigger discovery");
        helper.assertTrue(
                !LandmarkDiscoveryService.isVisitTransition(camp, null),
                "Leaving a landmark was incorrectly treated as a discovery");

        AdvancementHolder advancement =
                helper.getLevel().getServer().getAdvancements().get(LandmarkDiscoveryService.ADVANCEMENT);
        helper.assertTrue(advancement != null, "Landmark discovery advancement was not loaded");
        helper.assertTrue(
                advancement.value().criteria().containsKey(LandmarkDiscoveryService.CRITERION),
    ...
    private static LandmarkDefinition definition(String path, int weight, LandmarkCategory category) {
        return definition(path, "landmarks/" + path, LandmarkPlacementStyle.SURFACE, -32, 128, weight, category);
    }
    ...
    private static LandmarkDefinition definition(
        ...
        return new LandmarkDefinition(
                ...
                List.of(ResourceKey.create(
                        Registries.PROCESSOR_LIST, ResourceLocation.fromNamespaceAndPath("test", "processor"))),
                ResourceKey.create(
