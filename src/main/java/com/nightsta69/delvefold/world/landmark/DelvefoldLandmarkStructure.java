...
public final class DelvefoldLandmarkStructure extends Structure {
    ...
    protected Optional<GenerationStub> findGenerationPoint(GenerationContext context) {
        ...
        if (snapshot == null || !snapshot.settings().initialized() || snapshot.settings().terrainMode() == null) {
            return Optional.empty();
        }
        TerrainMode terrain = snapshot.settings().terrainMode();
        ChunkPos chunk = context.chunkPos();
        Holder<Biome> generatorBiome = context.biomeSource().getNoiseBiome(
                QuartPos.fromBlock(chunk.getMiddleBlockX()), 0,
                QuartPos.fromBlock(chunk.getMiddleBlockZ()), context.randomState().sampler());
        ...
        if (!LandmarkSelector.accepts(identity.landmarkPreset(),
                LandmarkSeeds.acceptanceSeed(context.seed(), chunk, generationSalt))) {
            return Optional.empty();
        }
        ...
        Optional<LandmarkGenerationPlanner.Selection<PlacementCandidate>> selected =
                LandmarkGenerationPlanner.select(
                        catalog,
                        terrain,
                        identity,
        ...
        BlockPos biomeCheckPosition = new BlockPos(
                chunk.getMiddleBlockX(), candidate.position().getY(), chunk.getMiddleBlockZ());
        return Optional.of(new GenerationStub(biomeCheckPosition, pieces -> pieces.addPiece(
                new LandmarkTemplatePiece(context.structureTemplateManager(), context.registryAccess(), definition,
                        candidate.position(), candidate.rotation(), candidate.contentSeed()))));
    }
    ...
    private static Optional<PlacementCandidate> candidate(
        ...
        ChunkPos chunk = context.chunkPos();
        long contentSeed = LandmarkSeeds.definitionSeed(
                context.seed(), chunk, generationSalt, definition.id());
        ...
        int y = probeCache.resolve(definition,
                () -> resolveY(context, definition, random, centerX, centerZ));
        ...
        Holder<Biome> biome = context.biomeSource().getNoiseBiome(
                QuartPos.fromBlock(centerX), QuartPos.fromBlock(y), QuartPos.fromBlock(centerZ),
    ...
    static BlockPos centeredOrigin(
        ...
        BlockPos desiredBoundsMinimum = new BlockPos(
                centerX - rotatedSize.getX() / 2,
                minimumY,
                centerZ - rotatedSize.getZ() / 2);
