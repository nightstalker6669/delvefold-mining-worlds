...
public final class GenerationSeedGameTests {
    ...
    public static void zeroSaltLandmarksPreserveLegacyDrawOrderAndPosition(GameTestHelper helper) {
        ...
        helper.assertTrue(
                expectedContentOrigin.equals(actualContentOrigin), "Modifier-to-feature landmark draw order changed");
        helper.assertTrue(
                expected.nextLong() == selected.nextLong(),
    ...
    public static void rejectedLegacyLandmarkConsumesOnlyTheRarityDraw(GameTestHelper helper) {
        ...
        expected.nextFloat();
        helper.assertTrue(
                GenerationSaltedLandmarkPlacement.candidateOrigin(actual, 99L, BlockPos.ZERO, 0L, 12)
                        .isEmpty(),
    ...
    public static void rotatedLandmarkPlacementAndContentAreRestartStableAndDomainSeparated(GameTestHelper helper) {
        ...
        for (int chunkX = -128; chunkX < 128; chunkX++) {
            ...
            GenerationSaltedLandmarkPlacement.candidateOrigin(passed, worldSeed, origin, 71L, 12)
                    .ifPresent(firstCandidates::add);
            ...
            GenerationSaltedLandmarkPlacement.candidateOrigin(RandomSource.create(chunkX), worldSeed, origin, 72L, 12)
                    .ifPresent(rotatedCandidates::add);
            helper.assertTrue(
                    passed.nextLong() == untouched.nextLong(),
                    "Rotated placement consumed the passed feature random in chunk " + chunkX);
        }
        helper.assertTrue(
                firstCandidates.equals(repeatedCandidates), "Persisted landmark placement salt was not deterministic");
        helper.assertTrue(
                !firstCandidates.equals(rotatedCandidates),
                "Changing generation salt did not rotate landmark candidates");

        ChunkPos chunk = new ChunkPos(-11, 37);
        RandomSource first = MiningLandmarkFeature.randomForContent(RandomSource.create(1L), 0x5EEDL, chunk, 71L);
        RandomSource repeated = MiningLandmarkFeature.randomForContent(RandomSource.create(2L), 0x5EEDL, chunk, 71L);
        RandomSource rotated = MiningLandmarkFeature.randomForContent(RandomSource.create(1L), 0x5EEDL, chunk, 72L);

        for (int draw = 0; draw < 16; draw++) {
            helper.assertTrue(
                    first.nextLong() == repeated.nextLong(),
                    "Persisted landmark salt was not deterministic at draw " + draw);
        }
        helper.assertTrue(
                MiningLandmarkFeature.randomForContent(RandomSource.create(1L), 0x5EEDL, chunk, 71L)
                                .nextLong()
                        != rotated.nextLong(),
    ...
    public static void saltedLandmarkModifierIsRegisteredAndLoadedFromData(GameTestHelper helper) {
        ...
        helper.assertTrue(
                placed.placement().size() == 1, "Mining landmarks must use exactly one combined placement modifier");
        helper.assertTrue(
                placed.placement().getFirst() instanceof GenerationSaltedLandmarkPlacement,
        ...
        helper.assertTrue(modifier.chance() == 12, "The bundled landmark placement chance was not decoded from JSON");
        helper.succeed();
    }
