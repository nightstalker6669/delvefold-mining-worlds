...
public final class DelvefoldGameTests {
    ...
    public static void outputsSharingAHostAreSelectedPerVein(GameTestHelper helper) {
        ...
        RuntimeOreProfile.CompiledBand band = profile.bands(
                        TerrainMode.FLAT, helper.getLevel().getBiome(helper.absolutePos(BlockPos.ZERO)))
                .getFirst();
    ...
    public static void tagWeightIsSharedAcrossSortedMembersAndOverlapsAreDeduplicated(GameTestHelper helper) {
        ...
        helper.assertTrue(
                group.outputs().size() == 3, "Overlapping exact output was not deduplicated from the host group");
        helper.assertTrue(
                group.outputs().get(0).state().getBlock() == Blocks.COAL_ORE,
                "Tag members were not sorted by registry ID");
        helper.assertTrue(
                group.outputs().get(1).state().getBlock() == Blocks.DEEPSLATE_COAL_ORE,
                "Tag members were not sorted by registry ID");
        helper.assertTrue(
                group.outputs().get(2).state().getBlock() == Blocks.DIAMOND_ORE,
                "Exact output did not retain target order after the sorted tag members");
        helper.assertTrue(
                group.outputs().get(0).selectionWeight() == 4.0D
                        && group.outputs().get(1).selectionWeight() == 4.0D,
                "The tag-level weight was not shared equally across its members");
        helper.assertTrue(
                group.outputs().get(2).selectionWeight() == 2.0D,
    ...
    public static void forecastUsesRuntimeTargetsBiomeAndDistributionMath(GameTestHelper helper) {
        ...
        OreProfileDocument document =
                new OreProfileDocument(OreProfileDocument.CURRENT_SCHEMA_VERSION, 7L, "forecast", List.of(rule));
        ...
        helper.assertTrue(
                ruleForecast.status() == OreProfileForecast.RuleStatus.EFFECTIVE,
                "A valid flat rule was not forecast as effective");
        helper.assertTrue(
                ruleForecast.effectiveAttempts() == 4.0D && ruleForecast.effectiveWorkUnits() == 32.0D,
                "Forecast workload did not match runtime band settings");
        helper.assertTrue(
                ruleForecast.effectiveOutputCount() == 1, "Forecast did not resolve the exact output and host tag");
        for (int y = 0; y <= 3; y++) {
            OreProfileForecast.HeightSample sample =
                    forecast.activeTerrainHeightOverlay().get(y + 64);
            helper.assertTrue(
                    sample.y() == y && sample.expectedAttempts() == 1.0D && sample.expectedWorkUnits() == 8.0D,
                    "Uniform height overlay was incorrect at Y=" + y);
        }
        helper.succeed();
    }

    private static RuntimeOreProfile.CompiledBand compileBand(GameTestHelper helper, List<OreTarget> targets) {
        OreRule rule = rule(targets);
        RuntimeOreProfile profile = RuntimeOreProfile.compile(
                new OreProfileDocument(OreProfileDocument.CURRENT_SCHEMA_VERSION, 0, "test", List.of(rule)));
        return profile.bands(TerrainMode.FLAT, helper.getLevel().getBiome(helper.absolutePos(BlockPos.ZERO)))
                .getFirst();
    }
