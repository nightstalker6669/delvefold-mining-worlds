...
final class LiveSettingsTransition {
    ...
    static List<ConfigIssue> validate(
        ...
        locked(
                issues,
                active.generationEpoch() == candidate.generationEpoch(),
                "settings.lifecycle.epoch_locked",
                "$.generation_epoch",
                "generation_epoch");
        locked(
                issues,
                active.generationSalt() == candidate.generationSalt(),
                "settings.lifecycle.salt_locked",
                "$.generation_salt",
                "generation_salt");
        locked(
                issues,
                active.initialized() == candidate.initialized(),
                "settings.lifecycle.initialized_locked",
                "$.initialized",
                "initialized");
        locked(
                issues,
                active.terrainMode() == candidate.terrainMode(),
                "settings.lifecycle.terrain_locked",
                "$.terrain_mode",
                "terrain_mode");
        locked(
                issues,
                active.identity().terrainVariant() == candidate.identity().terrainVariant(),
                "settings.lifecycle.variant_locked",
                "$.identity.terrain_variant",
                "terrain_variant");
        locked(
                issues,
                active.identity().geologyTheme() == candidate.identity().geologyTheme(),
                "settings.lifecycle.geology_theme_locked",
                "$.identity.geology_theme",
                "geology_theme");
        locked(
                issues,
                active.lastWorldOperationId().equals(candidate.lastWorldOperationId()),
                "settings.lifecycle.operation_locked",
                "$.last_world_operation_id",
                "last_world_operation_id");
        return List.copyOf(issues);
    }

    private static void locked(List<ConfigIssue> issues, boolean unchanged, String code, String path, String field) {
        if (!unchanged) {
