...
/**
 * Immutable horizontal location and protection boundary for the central mining hub.
 * The service chooses a safe vertical position for the active terrain at these coordinates.
 */
public record PortalHubSettings(
    ...
    public PortalHubSettings {
        ...
        if (protectionRadius < MIN_PROTECTION_RADIUS
                || protectionRadius > MAX_PROTECTION_RADIUS) {
            throw new IllegalArgumentException("Central hub protection radius must be between "
                    + MIN_PROTECTION_RADIUS + " and " + MAX_PROTECTION_RADIUS);
        }
    }
