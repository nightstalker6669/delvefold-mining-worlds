    ...
    public static ValidationReport validate(@Nullable WorldSettingsDocument settings) {
        ...
        if (settings == null) {
            return new ValidationReport(
                    List.of(ConfigIssue.error("settings.missing", "$", "Settings document is missing")));
        }
        ...
        if (settings.generationEpoch() < 0) {
            issues.add(ConfigIssue.error(
                    "settings.epoch.negative", "$.generation_epoch", "Generation epoch cannot be negative"));
        }
        if (settings.generationSalt() < 0) {
            issues.add(ConfigIssue.error(
                    "settings.generation_salt.negative", "$.generation_salt", "Generation salt cannot be negative"));
        }
        if (!settings.initialized() && settings.generationSalt() != 0L) {
            issues.add(ConfigIssue.error(
                    "settings.generation_salt.uninitialized",
                    "$.generation_salt",
                    "An uninitialized mining world cannot retain an active generation salt"));
        }
        if (settings.initialized() && settings.terrainMode() == null) {
            issues.add(ConfigIssue.error(
                    "settings.terrain.missing", "$.terrain_mode", "Initialized worlds require a terrain mode"));
        }
        if (!settings.initialized() && settings.terrainMode() != null) {
            issues.add(ConfigIssue.error(
                    "settings.terrain.unexpected",
                    "$.terrain_mode",
                    "Uninitialized worlds cannot select a terrain mode"));
        }
        if (settings.lastWorldOperationId().length() > 128) {
            issues.add(ConfigIssue.error(
                    "settings.operation_id.too_long",
        ...
        if (cooldown < 1 || cooldown > 3600) {
            issues.add(ConfigIssue.error(
                    "settings.portal.invalid_cooldown",
                    "$.portal.cooldown_seconds",
        ...
        if (!Double.isFinite(scale) || scale < 0.01D || scale > 100.0D) {
            issues.add(ConfigIssue.error(
                    "settings.portal.invalid_scale",
                    "$.portal.coordinate_scale",
