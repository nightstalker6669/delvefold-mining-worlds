...
public final class OreConfigValidator {
    ...
    public static ValidationReport validate(OreProfileDocument document, RegistryLookup registries) {
        ...
        if (document.schemaVersion() != OreProfileDocument.CURRENT_SCHEMA_VERSION) {
            issues.add(ConfigIssue.error(
                    "schema.unsupported",
                    "$.schema_version",
                    "Expected schema version " + OreProfileDocument.CURRENT_SCHEMA_VERSION + " but found "
        ...
        if (document.profile().length() > MAX_ID_LENGTH) {
            issues.add(ConfigIssue.error(
                    "profile.too_long", "$.profile", "Profile name cannot exceed " + MAX_ID_LENGTH + " characters"));
        }
        if (document.rules().size() > MAX_RULES) {
            issues.add(
                    ConfigIssue.error("rules.too_many", "$.rules", "At most " + MAX_RULES + " ore rules are allowed"));
        }
    ...
    private static void validateRule(OreRule rule, String path, RegistryLookup registries, List<ConfigIssue> issues) {
        if (rule.id().isBlank()
                || rule.id().length() > MAX_ID_LENGTH
                || !RULE_ID.matcher(rule.id()).matches()) {
            issues.add(ConfigIssue.error(
                    "rule.invalid_id", path + ".id", "Use lowercase letters, digits, dots, dashes, or underscores"));
        }
        if (rule.terrainModes().isEmpty()) {
            issues.add(
                    ConfigIssue.error("rule.no_terrain", path + ".terrain_modes", "Select at least one terrain mode"));
        }
        if (rule.targets().isEmpty()) {
            ...
            issues.add(ConfigIssue.error(
                    ...
                    path + ".targets",
                    "At most " + MAX_TARGETS_PER_RULE + " targets are allowed"));
        }
        if (rule.bands().isEmpty()) {
            ...
            issues.add(ConfigIssue.error(
                    "rule.too_many_bands", path + ".bands", "At most " + MAX_BANDS_PER_RULE + " bands are allowed"));
        }
        ...
        for (int i = 0; i < rule.targets().size(); i++) {
            ...
            if (stripHash(target.replaceTag()).length() > MAX_ID_LENGTH
                ...
                issues.add(ConfigIssue.error(
                        ...
                        targetPath + ".replace_tag",
                        "Host block tag is missing or empty: " + target.replaceTag()));
            }
            for (Map.Entry<String, String> property : target.state().entrySet()) {
                String propertyPath = targetPath + ".state." + property.getKey();
                if (property.getKey() == null
                        || property.getKey().isBlank()
                        || property.getKey().length() > MAX_ID_LENGTH) {
                    issues.add(ConfigIssue.error(
                            "target.invalid_state_property",
                            propertyPath,
                            "Block-state property names must be non-empty and at most " + MAX_ID_LENGTH
                                    + " characters"));
                }
                if (property.getValue() == null || property.getValue().length() > MAX_ID_LENGTH) {
                    issues.add(ConfigIssue.error(
                            "target.invalid_state_value",
                            propertyPath,
        ...
        for (int i = 0; i < rule.bands().size(); i++) {
            ...
            if (!bandIds.add(band.id())) {
                issues.add(ConfigIssue.error(
                        "band.duplicate_id", bandPath + ".id", "Duplicate band id within rule: " + band.id()));
            }
        }
    }
    ...
    private static void validateBand(SpawnBand band, String path, List<ConfigIssue> issues) {
        if (band.id().isBlank()
                || band.id().length() > MAX_ID_LENGTH
                || !RULE_ID.matcher(band.id()).matches()) {
            issues.add(ConfigIssue.error(
                    "band.invalid_id", path + ".id", "Use lowercase letters, digits, dots, dashes, or underscores"));
        }
        ...
        if (band.veinSize() < 1 || band.veinSize() > 64) {
            issues.add(ConfigIssue.error(
                    "band.invalid_vein_size", path + ".vein_size", "Vein size must be between 1 and 64"));
        }
        if (!Double.isFinite(band.attemptsPerChunk()) || band.attemptsPerChunk() < 0 || band.attemptsPerChunk() > 256) {
            issues.add(ConfigIssue.error(
                    "band.invalid_attempts", path + ".attempts_per_chunk", "Attempts must be between 0 and 256"));
        }
        if (band.placement() == OreBandPlacement.VEIN) {
            if (band.province() != null) {
                issues.add(ConfigIssue.error(
                        "band.unexpected_province", path + ".province", "Vein bands cannot contain province settings"));
            }
        ...
        if (band.minY() < MIN_WORLD_Y || band.maxY() > MAX_WORLD_Y || band.minY() > band.maxY()) {
            issues.add(ConfigIssue.error(
                    "band.invalid_height", path, "Height must stay within -64..320 and min_y cannot exceed max_y"));
        }
        if (!Double.isFinite(band.discardOnAirExposure())
                || band.discardOnAirExposure() < 0
                || band.discardOnAirExposure() > 1) {
            issues.add(ConfigIssue.error(
                    "band.invalid_air_discard",
        ...
        if (band.distribution() == HeightDistribution.TRAPEZOID) {
            ...
            if (low == null || high == null || low < band.minY() || high > band.maxY() || low > high) {
                issues.add(ConfigIssue.error(
                        "band.invalid_plateau", path, "Trapezoid plateau must be ordered and inside the height range"));
            }
        }
    }
    ...
    private static void validateProvince(ProvinceSettings province, String path, List<ConfigIssue> issues) {
        ...
        if (province.regionSize() < MIN_PROVINCE_REGION_SIZE
            ...
            issues.add(ConfigIssue.error(
                    ...
                    path + ".region_size",
                    "Region size must be a multiple of 16 from " + MIN_PROVINCE_REGION_SIZE + " through "
                            + MAX_PROVINCE_REGION_SIZE));
        }
        ...
        if (province.verticalThickness() < 1 || province.verticalThickness() > worldHeight) {
            issues.add(ConfigIssue.error(
                    ...
                    path + ".vertical_thickness",
                    "Province vertical thickness must be between 1 and " + worldHeight));
        }
        if (!Double.isFinite(province.density()) || province.density() <= 0.0D || province.density() > 1.0D) {
            issues.add(ConfigIssue.error(
                    "province.invalid_density",
                    path + ".density",
                    "Province density must be greater than 0 and at most 1"));
        }
        if (province.perChunkWorkCap() < 1 || province.perChunkWorkCap() > MAX_PROVINCE_WORK_PER_CHUNK) {
            issues.add(ConfigIssue.error(
                    "province.invalid_work_cap",
                    path + ".per_chunk_work_cap",
                    "Province per-chunk work cap must be between 1 and " + MAX_PROVINCE_WORK_PER_CHUNK));
        }
    }
    ...
    private static void validateBiomeSelectors(List<String> selectors, String path, List<ConfigIssue> issues) {
        if (selectors.size() > MAX_BIOME_SELECTORS) {
            issues.add(ConfigIssue.error(
                    "biomes.too_many", path, "At most " + MAX_BIOME_SELECTORS + " biome selectors are allowed"));
        }
        Set<String> unique = new HashSet<>();
        for (int index = 0; index < selectors.size(); index++) {
            String selector = selectors.get(index);
            String selectorPath = path + '[' + index + ']';
            if (selector == null || selector.length() > MAX_ID_LENGTH + 1 || !isResourceLocation(stripHash(selector))) {
                issues.add(ConfigIssue.error(
                        "biomes.invalid_selector", selectorPath, "Use a biome registry ID or #tag selector"));
            }
            if (!unique.add(selector)) {
                issues.add(ConfigIssue.warning(
                        "biomes.duplicate_selector", selectorPath, "Duplicate biome selector: " + selector));
            }
        }
    }
