...
public final class DelvefoldServerLifecycle {
    ...
    public static synchronized void register() {
        ...
        gameBus.addListener(EventPriority.HIGHEST, ServerAboutToStartEvent.class,
                event -> {
                    var saveRoot = event.getServer().getWorldPath(
    ...
    private static void applyBackupRetention(ServerAboutToStartEvent event) {
        ...
        try {
            ...
            for (var prune : preview.plan().prunes()) {
                LOGGER.info("Delvefold backup retention preview: prune {} for {} ({} bytes)",
                        prune.id(), prune.reasons(), prune.sizeBytes());
            ...
            result.failures().forEach((id, failure) ->
                    LOGGER.warn("Delvefold backup retention kept {}: {}", id, failure));
